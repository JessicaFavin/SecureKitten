...
public class KittenService extends Thread {
    ...
    private boolean authenticate() {
		...
		String 	password;
        do {
            this.userOutput.print("Username : ");
        ...
        this.userOutput.print("Password : ");
	...
	private void listCommands() {
		this.userOutput.println("/QUIT — to disconnect. \n"
		+"/USERS — to list users. \n"
		+"/SEND username message —  to send a message to username.\n"
		+"/SENDFILE filename username — to send file.\n"
		+"> ");
	}
	...
	private void unicastMessage(String input) {
		...
		if(cmd.length >= 3) {
			...
			if(id>-1){
				outputs.get(id).print(this.username+" : "+message+"\n> ");
			} else {
				this.info("No "+cmd[1]+" user logged.");
			}
		} else {
			this.info("/SEND username message — Not enough arguments");
		}
	}
